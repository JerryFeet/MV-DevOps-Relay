...
import {
  ...
  MessageSquare, Settings, Mail, AlertCircle, Lightbulb, Eye, Clock, Bot, FileText,
...
type VerificationItem = {
  ...
  titleDeedKey: string | null; ownerNationalId: string | null; parkingLots: string | null;
...
export default function AdminPage() {
  ...
  const attentionQueues = sortAttentionQueues(attentionQueueDefinitions.map((definition, originalIndex) => {
  ...
  const totalAttentionItems = pendingItems
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-2 p-4 sm:grid-cols-2 lg:grid-cols-3">
          {attentionQueues.map((queue) => {
            const isLoaded = pendingItems !== undefined;
                  ...
                  <div key={p.id} className="border border-slate-100 rounded-lg p-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{t(lang, "adm_unit_label")} {p.unitNumber ?? "—"} · {new Date(p.createdAt).toLocaleDateString(lang === "ar" ? "ar-SA" : "en-SA")}</p>
                  ...
                  <div key={f.id} className="border border-slate-100 rounded-lg p-3 flex items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <p className="text-sm font-medium text-slate-900 capitalize">{f.type?.replace("_", " ")} — {t(lang, "adm_unit_label")} {f.unitNumber}</p>
                      </div>
      ...
      <Section
        ...
        title={`${t(lang, "adm_communications")} (${communications.filter((c: any) => c.status === "pending").length})`}
      ...
      {/* HOA Settings */}
      <Section title={t(lang, "adm_settings")} defaultOpen={false}>
        <HoaSettingsForm
          current={hoaSettings ?? {}}
                ...
                return (
                  <div key={u.id} className="rounded-lg border border-slate-100 p-3">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-slate-500">
                      ...
                      {u.unitNumber && (
                        <span>{t(lang, "adm_unit_label")} {u.unitNumber}</span>
                      )}
                      {u.phone && (
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-slate-100 space-y-3 bg-slate-50">
          ...
          <div className="flex flex-wrap gap-2">
            {c.status !== "read" && !isClosed && (
              <Button size="sm" variant="outline" className="h-8 text-xs gap-1.5 text-blue-600 border-blue-200" onClick={() => onUpdate(c.id, "read", note)}>
                <Eye className="h-3.5 w-3.5" /> {t(lang, "adm_mark_read")}
              </Button>
            )}
            {!isClosed && (
              <Button size="sm" className="h-8 text-xs gap-1.5 bg-green-600 hover:bg-green-700" onClick={() => onUpdate(c.id, "resolved", note)}>
                <CheckCircle2 className="h-3.5 w-3.5" /> {t(lang, "adm_resolve")}
              </Button>
            )}
            {!isClosed && (
              <Button size="sm" variant="outline" className="h-8 text-xs gap-1.5 text-red-600 border-red-200 hover:bg-red-50" disabled={note.trim().length < 5} onClick={() => onUpdate(c.id, "rejected", note)}>
                <XCircle className="h-3.5 w-3.5" /> {t(lang, "adm_comm_reject")}
              </Button>
            )}
            {!isClosed && (
              <Button size="sm" variant="outline" className="h-8 text-xs gap-1.5 text-amber-600 border-amber-200 hover:bg-amber-50" disabled={note.trim().length < 5} onClick={() => onUpdate(c.id, "deferred_to_maintenance", note)}>
...
function HoaSettingsForm({
  ...
  const [waterPhone, setWaterPhone] = useState(current.water_phone ?? "");
  const [waterEmail, setWaterEmail] = useState(current.water_email ?? "");
  ...
  const [hoaEmail, setHoaEmail] = useState(current.hoa_email ?? "");
  ...
  return (
    <div className="mt-4 space-y-4 max-w-lg">
      ...
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 space-y-3">
        ...
        <div>
          <Label className="mb-1 block text-xs">{t(lang, "adm_water_phone")}</Label>
          <PhoneInput
            value={waterPhone}
            onChange={setWaterPhone}
            T={(key) => t(lang, key)}
          />
        </div>
        <div>
          <Label className="mb-1 block text-xs">{t(lang, "adm_water_email")}</Label>
          <Input
            type="email"
            placeholder="water@madainvillage.com"
            value={waterEmail}
            onChange={e => setWaterEmail(e.target.value)}
            className="bg-white"
          />
        </div>
        <div>
        ...
        <div>
          <Label className="mb-1 block text-xs">{t(lang, "adm_hoa_email")}</Label>
          <Input
            type="email"
            placeholder="hoa@madainvillage.com"
            value={hoaEmail}
...
function WahaPassSection({
  ...
  const allEvents = apps
    .flatMap((a: any) => (a.events ?? []).map((e: any) => ({ ...e, unitNumber: a.applicant?.unitNumber ?? "—", appId: a.id })))
          ...
          {unitApps.map(app => (
            <div key={app.id} className="border border-slate-100 rounded-lg p-3 mb-2 text-xs space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-semibold text-slate-800">{T("adm_unit_label")} {app.applicant?.unitNumber ?? "—"}</span>
                    ...
                    {pagedEvents.map((ev: any) => {
                      ...
                      return (
                        <tr key={ev.id} className="text-slate-700">
                          ...
                          <td className="px-3 py-2">{ev.unitNumber}</td>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-medium text-slate-900 mt-1">
            {T("adm_unit_label")} {app.applicant?.unitNumber ?? "—"}
          </p>
  ...
  return (
    <>
      <tr className="text-slate-700">
        <td className="px-3 py-2">{app.applicant?.unitNumber ?? "—"}</td>
...
function TitleDeedLink({ verificationId, lang }: { verificationId: number; lang: Lang }) {
  ...
  return (
    ...
    >
      {loading ? t(lang, "common_loading") : t(lang, "adm_view_title_deed")}
...
function VerificationRow({ v, onApprove, onReject }: {
  ...
  const basisOptions = [
    { value: "mullak_verified", label: t(lang, "sg11_mullak_verified") },
    { value: "title_deed_reviewed", label: t(lang, "sg11_title_deed_reviewed") },
    ...
    <div className="border border-slate-100 rounded-lg p-4">
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {t(lang, "adm_unit_label")} <strong>{v.unit ? `${v.unit.building} ${v.unit.unitNumber}` : "—"}</strong>
            {v.nationalId && <> · {t(lang, "adm_id_label")} {v.nationalId}</>}
            {v.ejarReference && <> · {t(lang, "adm_ejar_label")} {v.ejarReference}</>}
          </p>
          {(v.firstName || v.lastName || v.mobile) && (
            <p className="text-xs text-slate-500 mt-0.5">
              {(v.firstName || v.lastName) && (
                <>[{v.firstName} {v.lastName}]</>
              )}
              {(v.firstName || v.lastName) && v.mobile && <> · </>}
              {v.mobile && <>{t(lang, "adm_mobile_label")} <strong>{v.mobile}</strong></>}
            </p>
          )}
...
function VerificationHistoryRow({ verification }: { verification: VerificationItem }) {
  ...
  const unitLabel = verification.unit ? `${verification.unit.building} ${verification.unit.unitNumber}` : "—";
