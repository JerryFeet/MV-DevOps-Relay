...
describe("POST /api/residents — portal access invitation", () => {
  ...
  it("persists the guardian identifier flag only for an under-18 resident", async () => {
    stores.residents.insert({
      type: "owner", firstName: "Dana", lastName: "Al-Harbi", unitNumber: "201", unitId: UNIT_201_ID,
      relationship: "Primary", idNumber: "PRIMARY-IDENTIFIER", dateOfBirth: "1980-01-01",
      hasPortalAccess: false, registeredById: 1, linkedUserId: 1, status: "active", isPrimary: true,
    });
    mockAuthState.userId = CLERK_OWNER;
  ...
  it("derives a minor guardian identifier from the primary occupant instead of the submitted value", async () => {
    stores.residents.insert({
      type: "owner", firstName: "Dana", lastName: "Al-Harbi", unitNumber: "201", unitId: UNIT_201_ID,
      relationship: "Primary", idNumber: "PRIMARY-IDENTIFIER", dateOfBirth: "1980-01-01",
      hasPortalAccess: false, registeredById: 1, linkedUserId: 1, status: "active", isPrimary: true,
    ...
    mockAuthState.userId = CLERK_OWNER;
    const res = await request(app).post("/api/residents").send({
      ...basePayload, firstName: "Protected Minor", dateOfBirth: "2015-05-05",
      idNumber: "UNTRUSTED-BROWSER-VALUE", idNumberIsGuardian: true,
    });
    expect(res.status).toBe(201);
    expect(res.body).toMatchObject({ idNumber: "PRIMARY-IDENTIFIER", idNumberIsGuardian: true });
  });

  it("keeps a fifth resident request visible as a read-only pending-verification list item", async () => {
    stores.residents.insert({
      type: "owner", firstName: "Dana", lastName: "Al-Harbi", unitNumber: "201", unitId: UNIT_201_ID,
      relationship: "Primary", idNumber: "PRIMARY-IDENTIFIER", dateOfBirth: "1980-01-01",
      hasPortalAccess: false, registeredById: 1, linkedUserId: 1, status: "active", isPrimary: true,
    ...
    stores.residents.insert({
      type: "family", firstName: "Third", lastName: "Member", unitNumber: "201", unitId: UNIT_201_ID,
      relationship: "Child", idNumber: "THIRD-ID", dateOfBirth: "2010-01-01",
      hasPortalAccess: false, registeredById: 1, status: "active", isPrimary: false,
    });
    mockAuthState.userId = CLERK_OWNER;
    const created = await request(app).post("/api/residents").send({
      ...basePayload, firstName: "Pending", lastName: "Review", reason: "Additional dependent", proofWarningAcknowledged: true,
    });
    expect(created.status).toBe(201);
    expect(stores.residents.findAll()).toHaveLength(4);
    const listed = await request(app).get("/api/residents");
    ...
    expect(listed.body.data).toEqual(expect.arrayContaining([
      expect.objectContaining({
        ...
        pendingReviewRequestId: created.body.id,
      }),
    ]));
  });
