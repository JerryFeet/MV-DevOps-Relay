...
function UnitOptionPicker({ options, value, onChange, colors, testID }: {
  ...
  return <ScrollView horizontal showsHorizontalScrollIndicator={false} testID={testID}>
    <View style={{ flexDirection: "row", gap: 8 }}>
      {options.map(option => <Pressable key={option} onPress={() => onChange(option)}
        style={{ minWidth: 38, paddingHorizontal: 10, paddingVertical: 10, borderRadius: 8, borderWidth: 1,
          ...
          backgroundColor: value === option ? colors.primary : colors.muted }}>
    ...
    <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      ...
      {item.unitNumber ? (
        <View style={[s.metaRow, isRTL && s.metaRowRTL]}>
          ...
          <Text style={[s.metaText, { color: colors.mutedForeground }]}>Unit {displayUnitReference(item.unitNumber)}</Text>
        </View>
      ) : null}
...
function MoveFormSheet({
  ...
  const unitNumber = building && apartment ? composeUnitReference(building, apartment) : "";
  ...
  return (
    <BottomSheet visible={visible} onClose={handleClose} colors={colors}>
      ...
      <ScrollView style={s.sheetBody} showsVerticalScrollIndicator={false}>
        ...
        <Text testID="move-label-building" style={[s.fieldLabel, { color: colors.mutedForeground }]}>{t("move_form_building_label")}</Text>
        <UnitOptionPicker options={BUILDING_OPTIONS} value={building} onChange={setBuilding} colors={colors} testID="move-building-selector" />
        <Text testID="move-label-apartment" style={[s.fieldLabel, { color: colors.mutedForeground }]}>{t("move_form_apartment_label")}</Text>
        <UnitOptionPicker options={APARTMENT_OPTIONS} value={apartment} onChange={setApartment} colors={colors} testID="move-apartment-selector" />
