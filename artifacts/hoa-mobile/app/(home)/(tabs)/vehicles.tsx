...
import { customFetch, listMyVehicleParkingLots, listVehicles } from "@workspace/api-client-react";
    ...
    <View style={[s.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
      <View style={[s.cardRow, isRTL && s.rowRTL]}>
        ...
        <View style={s.vehicleInfo}>
          ...
          {item.parkingLot && <Text style={[s.colorText, { color: colors.mutedForeground }]}>{item.parkingLot.lotNumber} · {item.parkingLot.parkingType === "underground" ? "Underground" : "Surface"}</Text>}
        </View>
      </View>
    </View>
  );
...
export default function VehiclesScreen() {
  ...
  const { data: parkingLots = [], isLoading: parkingLotsLoading, isError: parkingLotsError } = useQuery({
    queryKey: ["vehicle-parking-lots"],
    queryFn: () => listMyVehicleParkingLots(),
  });
  ...
  const handleRegister = async () => {
    if (!plateNumber || !make || !model || !parkingLotId) return;
    ...
    const body: Record<string, unknown> = { plateNumber, make, model, parkingLotId };
    ...
    <View style={[s.container, { backgroundColor: colors.background }]}>
      ...
      <BottomSheet visible={showSheet} onClose={handleClose} colors={colors}>
        ...
        <ScrollView style={s.sheetBody} showsVerticalScrollIndicator={false}>
          ...
          <Text style={[s.fieldLabel, { color: colors.foreground }]}>{t("veh_parking_lot")} *</Text>
          {parkingLotsLoading ? <ActivityIndicator color={colors.primary} /> : parkingLotsError ? (
            <Text style={[s.errorText, { color: "#dc2626" }]}>{t("veh_parking_load_error")}</Text>
          ) : (
            <View style={s.lotChoices} testID="vehicle-parking-lots">
              {parkingLots.map((lot) => (
                <Pressable key={lot.id} onPress={() => setParkingLotId(lot.id)}
                  style={[s.lotChoice, { borderColor: parkingLotId === lot.id ? colors.primary : colors.border, backgroundColor: parkingLotId === lot.id ? colors.primary + "16" : colors.muted }]}>
                  <Text style={{ color: colors.foreground }}>{lot.lotNumber} · {lot.underground ? t("veh_parking_underground") : t("veh_parking_surface")}</Text>
                </Pressable>
              ))}
            </View>
          )}
          ...
          <Pressable
            style={({ pressed }) => [
              ...
              { backgroundColor: colors.primary },
               (!plateNumber || !make || !model || !parkingLotId || parkingLotsLoading || createMutation.isPending) && { opacity: 0.5 },
              pressed && { opacity: 0.8 },
            ]}
            onPress={handleRegister}
             disabled={!plateNumber || !make || !model || !parkingLotId || parkingLotsLoading || !!parkingLotsError || createMutation.isPending}
          >
...
const s = StyleSheet.create({
  ...
  lotChoices: { gap: 8 },
  lotChoice: { borderWidth: 1, borderRadius: 10, padding: 12 },
