...
router.post("/permits", requireApiAuth, async (req, res) => {
  ...
  const unitLabel = ` — Unit ${unitReference}`;
...
router.patch("/permits/:id/status", requireApiAuth, async (req, res) => {
  ...
  // A move-out completion is atomic with the household release.  Do not first
  // mark the permit terminal and then attempt a separate cleanup transaction.
  if (status === "completed" && existing.type === "move_out") {
    if (existing.userId === null) {
      return res.status(409).json({ error: "The move-out permit no longer has an active household subject." });
    }
    try {
      const result = await executeMoveOutPermitRelease(existing.userId, caller.id, existing.id);
      if (result && result.outcome !== "released" && result.outcome !== "already_ended") {
        return res.status(409).json({ error: "The move-out permit could not complete the household release.", result });
      }
      const [completed] = await db.select().from(permitsTable).where(eq(permitsTable.id, existing.id));
      const [enriched] = await enrichPermits(completed ? [completed] : []);
      return res.json(enriched);
    } catch (error) {
      return res.status(409).json({
        error: error instanceof Error ? error.message : "The move-out permit could not complete the household release.",
      });
    }
  }
    ...
    if (pushDef && permit.userId !== null) {
      ...
      if (DECISION_STATUSES.has(status)) {
        enqueueBothNotificationChannels({
          ...
          recipientEmail: (enriched as { requester?: { email?: string | null } }).requester?.email ?? null,
          payload: {
            title: pushDef.title,
            body: pushDef.body(typeLabel, reviewNote ?? null),
            subject: `${typeLabel}: ${status}`,
            html: `<p>${pushDef.body(typeLabel, reviewNote ?? null)}</p>`,
